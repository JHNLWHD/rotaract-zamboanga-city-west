              ...
              <div className="bg-white rounded-xl shadow-lg border border-cranberry-100 p-8">
                <div className="max-w-2xl mx-auto space-y-3">
                  {pastPresidents.map((president, index) => (
                    <div key={index} className="flex justify-between items-center py-3 px-4 rounded-lg hover:bg-cranberry-50 transition-colors border-b border-gray-100">
                      <span className="font-medium text-slate-700">{president.term}</span>
                      <div className="text-right">
                        <span className="text-slate-900 font-semibold">{president.name}</span>
                        {president.status === 'current' && (
                          <span className="block text-xs text-cranberry-600 mt-1">Current President</span>
                        )}
                        {president.status === 'president_elect' && (
                          <span className="block text-xs text-blue-600 mt-1">President-Elect</span>
                        )}
                        {president.status === 'future' && (
                          <span className="block text-xs text-green-600 mt-1">President-Nominee</span>
                        )}
                      </div>
                    </div>
                  ))}
